...
class MultiServerService(ExternalService, IDisposable):
    ...
    def SetServerState(self, serverId, document, bActive):
        """
        SetServerState(self: MultiServerService, serverId: Guid, document: Document, bActive: bool) -> bool
        
            This method will simply switch the active state of a server without affecting 
             in any way the other servers.
        
        
            serverId: Id of the server to switch active state for.
            document: Document for which to activate this server. If null, server will activate 
             globally.
        
            bActive: True to activate server, false to deactivate.
            Returns: True if operation succeeded (even if the server state was not changed), false 
             otherwise (e.g. when serverId is invalid).
        """
        pass
